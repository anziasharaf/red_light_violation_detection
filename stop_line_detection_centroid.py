...
def clip_video(ti):
   ...
   ffmpeg_extract_subclip("/home/user/Desktop/Red_light_violation/Red_light_violation_videos/video_1.mp4",int(ti)-5, int(ti), targetname="test.mp4")
offset=100
violation=False
def getFirstFrame(videofile):
    ...
    if success:
        cv2.imwrite("/home/user/Desktop/stop_line_detection/Vehicle_Detection/RLV/RLV_2023-01-23_10:22:42.924662/first_frame_106.png", image)
        image_name="/home/user/Desktop/stop_line_detection/Vehicle_Detection/RLV/RLV_2023-01-23_10:22:42.924662/first_frame_106.png"
        return image_name
class DrawLineWidget(object):
    def __init__(self):
        self.original_image = cv2.imread('/home/user/Desktop/projects/stop_line_detection/Vehicle_Detection/preview/first_frame_106.png')
        #self.clone = self.original_image.copy()
        self.resize=cv2.resize(self.original_image,(640,640))
...
video_path='rtsp://admin:123456@192.168.2.106:554'
cap = cv2.VideoCapture(video_path)
#img_path=getFirstFrame(video_path)
#print(img_path)
points=get_points()
print(points)
...
while True:
	...
	x_cord1=points[0][0]
	x_cord2=points[0][1]
	x_mid=(x_cord1+x_cord2)/2
	y_mid=(y_cord1+y_cord2)/2
	print(x_mid,y_mid)
	...
	for i in indices:
		if classes_ids[i] in interested_class_ids:
			...
			y_mid=(y_cord1+y_cord2)/2
			print(x_mid,y_mid)
			# cv2.circle(img,center=(x_mid,y_mid),radius=10,color=(255,0,0),thickness=10)
			if value<0:
				if x_cord1<c_x<x_mid :
					if y_mid>c_y>y_cord2:
                       
                            # print(x1,y1,w,h)
						conf = confidences[i]
						text = label + "{:.2f}".format(conf)
						cv2.rectangle(img,(x1,y1),(x1+w,y1+h),(0,0,255),2)
						print("detected cordinates",x1,y1,w,h)
				...
				elif x_mid<c_x<x_cord2:
					if y_mid>c_y>y_cord2-50:
